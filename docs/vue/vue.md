...
1. 什么是组件化一句话来说就是把图形、非图形的各种逻辑均抽象为一个统一的概念（组件）来实现开发的模式，在Vue中每一个`.vue`文件都可以视为一个组件
2. 组件化的优势
...
用Jquery来实现大概的思路就是选择流程dom对象，点击按钮隐藏当前活动流程dom对象，显示下一流程dom对象如下图
...
- 都有自己的构建工具：`Vue`的`vue-cli`（现在基本已经不使用了，创建过程为`yarn create vue`）、`React`的`Create React App`
